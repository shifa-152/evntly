...
const offerSchema = new mongoose.Schema({
  ...
  title:         { type: String, required: true, maxlength: 80 },
  description:   { type: String, default: '' },
  discountType:  { type: String, enum: ['percent','flat','free_amenity','custom','poster'], default: 'percent' },
  discountValue: { type: Number, default: 0 },
  validFrom:     { type: String, default: null },
  validTill:     { type: String, default: null },
  image:         { type: String, default: null },
...
// Returns true if an offer's "validTill" date has already passed (auto-expiry)
function isOfferExpired(offer) {
  if (!offer.validTill) return false;
  const today = new Date().toISOString().split('T')[0];
  return offer.validTill < today;
}
// ─── HELPERS ──────────────────────────────────────────────────────────────────
function genRef() {
  return 'EVT' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).substring(2,5).toUpperCase();
...
app.get('/api/offers', async (req, res) => {
  try {
    ...
    // Attach a computed "isExpired" flag based on validTill (auto-expiry)
    offers = offers.map(o => {
      const obj = o.toObject();
      obj.isExpired = isOfferExpired(o);
      return obj;
    });

    // When the public/active feed is requested, hide offers that have expired
    if (req.query.active === 'true') {
      offers = offers.filter(o => !o.isExpired);
    }
...
app.post('/api/offers', ownerMiddleware, upload.single('image'), async (req, res) => {
  try {
    ...
    if (!venueId) return res.status(400).json({ error: 'venueId is required' });
    if (!title)   return res.status(400).json({ error: 'title is required' });
    if ((discountType || 'percent') === 'poster' && !req.file)
      return res.status(400).json({ error: 'Please upload a poster image for this offer' });
