...
async function openVenueDetails(venueId) {
  ...
  try {
    v = await api('/venues/' + venueId + '?_=' + Date.now());
    offers = await api('/offers?active=true&venueId=' + venueId).catch(() => []);
  } catch(e) {
    toast('Could not load venue details', 'error');
    return;
  }
  ...
  const titleEl = document.getElementById('booking-modal-title');
  if (titleEl) titleEl.textContent = v.name;
  renderVenueDetailsOnly(v, offers);
  openModal('booking-modal');
}

function renderVenueOffersHtml(offers) {
  const active = (offers || []).filter(o => o.active && !o.isExpired);
  if (!active.length) return '';
  const cards = active.map(o => {
    const discLabel = o.discountType === 'percent'      ? `${o.discountValue}% Off`
                    : o.discountType === 'flat'         ? `₹${Number(o.discountValue||0).toLocaleString('en-IN')} Off`
                    : o.discountType === 'free_amenity' ? 'Free Add-on'
                    : o.discountType === 'poster'       ? '📋 Package Menu'
                    : 'Special Offer';
    const imgHtml = o.image
      ? `<img src="${imgUrl(o.image)}" alt="${escHtml(o.title)}" style="width:100%;max-height:420px;object-fit:contain;background:var(--cream);border-radius:8px;display:block;cursor:pointer" onclick="window.open('${imgUrl(o.image)}','_blank')" onerror="this.style.display='none'" />`
      : '';
    return `
      <div style="border:1px solid var(--border);border-radius:8px;overflow:hidden;margin-bottom:14px">
        ${imgHtml}
        <div style="padding:12px 16px">
          <span class="offer-poster-badge">🏷️ ${discLabel}</span>
          <div style="font-family:'Playfair Display',serif;font-weight:700;font-size:1rem;color:var(--dark);margin-bottom:2px">${escHtml(o.title)}</div>
          ${o.description ? `<div style="font-size:0.85rem;color:#555;line-height:1.55">${escHtml(o.description)}</div>` : ''}
          ${(o.validFrom || o.validTill) ? `<div style="font-size:0.75rem;color:var(--muted);margin-top:6px">📅 Valid ${o.validFrom || '—'} to ${o.validTill || '—'}</div>` : ''}
        </div>
      </div>`;
  }).join('');
  return `<div class="vd-amenities"><div class="vd-section-title">🎁 Current Offers &amp; Packages</div>${cards}</div>`;
}

function renderVenueDetailsOnly(v, offers) {
  const allImages = [];
  if (v.coverImage) allImages.push(v.coverImage);
  (v.images || []).forEach(function(img) { if (img && !allImages.includes(img)) allImages.push(img); });
...
async function openBookingModal(venueId) {
  ...
  try {
    const titleEl = document.getElementById('booking-modal-title');
    if (titleEl) titleEl.textContent = v.name;
    renderVenueDetailModal(v, offers);
    openModal('booking-modal');
  } catch(e) {
    console.error('renderVenueDetailModal crash:', e);
    toast('UI error rendering venue: ' + (e?.message || String(e)), 'error');
...
function renderVenueDetailModal(v, offers) {
  const allImages = [];
  if (v.coverImage) allImages.push(v.coverImage);
  (v.images || []).forEach(function(img) { if (img && !allImages.includes(img)) allImages.push(img); });
...
async function loadOffersPanel() {
  ...
  container.innerHTML = `
    <style>
      ...
      .offer-list-card{background:#fff;border:1px solid var(--border,#e5e5e5);border-radius:10px;overflow:hidden;margin-bottom:14px;transition:box-shadow .15s}
      .offer-list-card:hover{box-shadow:0 4px 18px rgba(0,0,0,0.07)}
      .offer-list-img{width:100%;max-height:320px;object-fit:contain;background:var(--cream,#f9f5ee);display:block}
      .offer-list-img-placeholder{width:100%;height:140px;display:flex;align-items:center;justify-content:center;font-size:2.5rem;background:var(--cream,#f9f5ee)}
      .offer-list-body{padding:14px 18px}
      .offer-list-title{font-family:'Playfair Display',serif;font-size:1rem;font-weight:700;color:var(--dark);margin-bottom:2px}
      .offer-list-venue{font-size:0.78rem;color:var(--muted);margin-bottom:8px}
      ...
      .offer-list-meta{display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-bottom:10px}
      .offer-meta-chip{font-size:0.73rem;font-weight:600;padding:3px 10px;border-radius:12px;background:var(--cream,#f9f5ee);color:var(--muted);border:1px solid var(--border,#e5e5e5)}
      .offer-meta-chip.expired{background:rgba(239,68,68,0.1);color:#ef4444;border-color:rgba(239,68,68,0.3)}
      .offer-list-footer{display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap}
      .offer-toggle-wrap{display:flex;align-items:center;gap:8px;font-size:0.8rem;color:var(--muted);font-weight:600}
      .offer-toggle{position:relative;display:inline-block;width:42px;height:22px;cursor:pointer}
      .offer-toggle input{display:none}
    ...
    <div class="offer-form-card">
      ...
      <div class="offer-form-grid">
        <div class="dash-form-field">
          ...
          <select class="dash-input" id="of-dtype" onchange="toggleOfferDiscount()">
            ...
            <option value="flat">Flat Amount Off (₹)</option>
            <option value="free_amenity">Free Amenity / Add-on</option>
            <option value="poster">📋 Poster / Package Menu (image only)</option>
            <option value="custom">Custom / Other</option>
          </select>
        </div>
        <div class="dash-form-field" id="of-dval-wrap">
      ...
      <div class="dash-form-field">
        <label class="dash-label" id="of-img-label">Offer Image <span style="font-weight:400;color:var(--muted)">(optional)</span></label>
        <div style="font-size:0.75rem;color:var(--muted);margin:-2px 0 8px">📐 Tip: for poster-style images (like a full package menu graphic), a tall/portrait image works best — it will be shown uncropped.</div>
        <div class="img-upload-area" style="padding:14px" onclick="document.getElementById('of-img-input').click()">
          <input type="file" id="of-img-input" accept="image/jpeg,image/png,image/webp" style="display:none" onchange="previewOfferImg(event)" />
          <div class="img-upload-icon" style="font-size:1.3rem">🖼️</div>
          <div class="img-upload-text" style="font-size:0.78rem" id="of-img-hint">Click to upload banner image</div>
        </div>
        <div id="of-img-preview" style="margin-top:8px"></div>
      </div>
      <button class="btn-submit" style="margin-top:4px;width:auto;padding:11px 30px" id="of-submit-btn" onclick="submitOffer()">Create Offer →</button>
...
function toggleOfferDiscount() {
  const dtype = document.getElementById('of-dtype')?.value;
  const wrap  = document.getElementById('of-dval-wrap');
  const imgLabel = document.getElementById('of-img-label');
  const imgHint  = document.getElementById('of-img-hint');
  if (!wrap) return;
  if (dtype === 'free_amenity' || dtype === 'custom' || dtype === 'poster') {
    wrap.style.display = 'none';
  } else {
    wrap.style.display = '';
    document.getElementById('of-dval').placeholder =
  ...
  if (dtype === 'poster') {
    if (imgLabel) imgLabel.innerHTML = 'Offer Image <span style="font-weight:400;color:#ef4444">(required)</span>';
    if (imgHint)  imgHint.textContent = 'Click to upload your poster / package menu image';
  } else {
    if (imgLabel) imgLabel.innerHTML = 'Offer Image <span style="font-weight:400;color:var(--muted)">(optional)</span>';
    if (imgHint)  imgHint.textContent = 'Click to upload banner image';
  }
}
...
async function submitOffer() {
  ...
  if (!venueId)    errs.push('Please select a venue');
  if (!title)      errs.push('Offer title is required');
  if (dtype !== 'free_amenity' && dtype !== 'custom' && dtype !== 'poster' && (!dval || parseFloat(dval) <= 0))
    errs.push('Please enter a discount value greater than 0');
  if (dtype === 'percent' && parseFloat(dval) > 100)
    errs.push('Percentage discount cannot exceed 100%');
  if (dtype === 'poster' && !ofImgFile)
    errs.push('Please upload a poster image for this offer');
  if (from && till && from > till)
    errs.push('"Valid From" must be before "Valid Till"');
  if (errs.length) {
    errBox.innerHTML = errs.map(e => `• ${e}`).join('<br>');
...
function renderOfferCards(offers, venues) {
  return offers.map(o => {
    ...
    const discLabel = o.discountType === 'percent'      ? `${o.discountValue}% Off`
                    : o.discountType === 'flat'         ? `₹${Number(o.discountValue).toLocaleString('en-IN')} Off`
                    : o.discountType === 'free_amenity' ? 'Free Add-on'
                    : o.discountType === 'poster'       ? '📋 Poster / Package'
                    : 'Special Offer';
    const dateRange = (o.validFrom || o.validTill)
      ? `📅 ${o.validFrom || '—'} → ${o.validTill || '—'}`
      : '📅 No expiry set';
    const imgHtml = o.image
      ? `<img class="offer-list-img" src="${imgUrl(o.image)}" alt="${escHtml(o.title)}" onerror="this.style.display='none'" />`
      : `<div class="offer-list-img-placeholder">🎁</div>`;
    const expiredChip = o.isExpired ? `<span class="offer-meta-chip expired">⏰ Expired</span>` : '';
    return `
      <div class="offer-list-card" id="ocard-${o._id}">
        ${imgHtml}
        <div class="offer-list-body">
          ...
          <div class="offer-list-footer">
            <label class="offer-toggle-wrap">
              ...
              <span id="offer-status-${o._id}">${o.isExpired ? 'Expired' : (o.active ? 'Active' : 'Paused')}</span>
            </label>
            <button class="offer-delete-btn" onclick="deleteOffer('${o._id}','${escHtml(o.title)}')">🗑️ Delete</button>
          </div>
        </div>
...
// ─── SPECIAL OFFERS SECTION (homepage — permanent gallery) ────────
async function loadSpecialOffersSection() {
  const section = document.getElementById('special-offers-section');
  const grid    = document.getElementById('special-offers-grid');
  ...
  try {
    ...
    grid.innerHTML = active.map(o => {
      const discLabel = o.discountType === 'percent'      ? `${o.discountValue}% Off`
                      ...
                      : 'Special Offer';
      const dateRange = (o.validFrom || o.validTill)
        ? `📅 Valid ${o.validFrom || '—'} to ${o.validTill || '—'}`
        : '';
      const imgHtml = o.image
        ? `<img class="offer-poster-img" src="${imgUrl(o.image)}" alt="${escHtml(o.title)}" onerror="this.style.display='none'" />`
        : '';
      return `
        <div class="offer-poster-card" onclick="openVenueDetails('${o.venueId}')">
          ${imgHtml}
          <div class="offer-poster-body">
            <span class="offer-poster-badge">🏷️ ${discLabel}</span>
            <div class="offer-poster-title">${escHtml(o.title)}</div>
            <div class="offer-poster-venue">📍 ${escHtml(o.venueName || 'EVNTLY Venue')}</div>
            ${o.description ? `<div class="offer-poster-desc">${escHtml(o.description)}</div>` : ''}
            ${dateRange ? `<div class="offer-poster-dates">${dateRange}</div>` : ''}
          </div>
        </div>`;
    }).join('');
